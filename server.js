...
async function createDefaultUsers() {
  try {
    const users = [
      { username: "manager_user", password: "Cenzo@385", role: "manager" },
      { username: "clerk_user", password: "Cenzo@495", role: "clerk" },
      {
        username: "accountant_user",
        password: "Cenzo@8245",
        role: "accountant",
      },
      { username: "admin_user", password: "Cenzo@3854958245", role: "admin" },
    ];
    ...
    for (const user of users) {
      ...
      if (!existingUser) {
        ...
        console.log(`Hashed password for ${user.username}:`, hashedPassword); // Log hashed password
        await User.create({
          username: user.username,
          password: hashedPassword,
          role: user.role,
        });
        console.log(`Created new default user: ${user.username}`);
      } else {
...
app.get(
  "/products",
  authenticateToken,
  authorizeRole("admin", "manager", "clerk", "accountant", "subDepotManager"),
  async (req, res) => {
    try {
...
app.post(
  "/stock-movements",
  authenticateToken,
  authorizeRole("admin", "manager", "clerk"),
  async (req, res) => {
    try {
...
app.get(
  ...
  authenticateToken,
  authorizeRole("admin", "manager", "accountant"),
  async (req, res) => {
    try {
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error retrieving stock movements", error });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      for (const role of rolesToNotify) {
        ...
        sendNotification(
          role,
          `New order placed by sub-depot manager ${req.user.id}. Please review.`
        );
      }
...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    ...
    console.log("Found user:", { username: user.username, role: user.role }); // Debug log
